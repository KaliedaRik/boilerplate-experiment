...
const doTests = (items, value) => {
	...
	let i, iz, itemKeys, item, type, test, temp,
		flag = true;
...
const checkForExpectedFields = (formName, values) => {
	let form, formKeys, valueKeys,
		i, iz, key,
		flag = true;
console.log('checkForExpectedFields 1', formName);
	if(!formName || !values) return false;
console.log('checkForExpectedFields 2');

	form = validations[formName] || [];
	formKeys = Object.keys(form);
	valueKeys = Object.keys(values);
console.log('checkForExpectedFields 3', formKeys, valueKeys);

	for(i = 0, iz = valueKeys.length; i < iz; i++){
		key = valueKeys[i];
console.log(`checkForExpectedFields 4.${i}`, key);
		if(formKeys.indexOf(key) < 0){
			flag = false;
			break;
		}
	}
console.log('checkForExpectedFields 5', flag);
	return flag;
};
